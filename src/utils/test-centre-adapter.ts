...
class TestCentreAdapter implements Adapter<TestCentre> {
  adapt(account: CRMAccount): TestCentre {
    return {
      ...
      ftts_tcntestcentreid: account.ftts_tcntestcentreid,
    };
  }
  ...
  private mapRegion(account: CRMAccount): TestCentreRegion | null {
    if (account.parentaccountid.ftts_regiona) return TestCentreRegion.A;
    if (account.parentaccountid.ftts_regionb) return TestCentreRegion.B;
    if (account.parentaccountid.ftts_regionc) return TestCentreRegion.C;
    return null;
  }

  private mapState(account: CRMAccount): Region | null {
    const remitMap = {
      [Remit.NorthernIreland]: Region.NI,
      [Remit.England]: Region.GB,
      [Remit.Wales]: Region.GB,
      [Remit.Scotland]: Region.GB,
    };
